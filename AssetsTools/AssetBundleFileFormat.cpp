...
// ============================================================
// UABE_AOV: AES-128-CBC helper (Windows BCrypt)
// Used only for AOV bundle header decryption (flag 0x200).
// ============================================================
#ifdef _WIN32
#include <windows.h>
#include <bcrypt.h>
...
static bool AovAES128CBC_Decrypt(
    const uint8_t* key16, const uint8_t* iv16,
    const uint8_t* ciphertext, uint8_t* plaintext, ULONG len)
{
    // All locals declared at top to avoid C2362 "init skipped by goto"
    ...
    if (!BCRYPT_SUCCESS(status)) goto aov_cleanup;
    ...
    keyBlob.hdr.cbKeyData = 16;
    memcpy(keyBlob.key, key16, 16);

    status = BCryptImportKey(hAlg, NULL, BCRYPT_KEY_DATA_BLOB, &hKey,
                             NULL, 0, (PUCHAR)&keyBlob, (ULONG)sizeof(keyBlob), 0);
    if (!BCRYPT_SUCCESS(status)) goto aov_cleanup;

    memcpy(iv_copy, iv16, 16);
    memcpy(plaintext, ciphertext, len);

    status = BCryptDecrypt(hKey, plaintext, len, NULL,
                           iv_copy, 16, plaintext, len, &cbResult, 0);
...
ASSETSTOOLS_API bool AssetBundleBlockAndDirectoryList06::Read(QWORD filePos, IAssetsReader *pReader, AssetsFileVerifyLogger errorLogger)
{
	...
	if (!blockInf)
		goto __goto_allocerror;
	// UABE_AOV: AOV block info order = flags(2) + unknown(2) + compressedSize(4) + decompressedSize(4)  [12 bytes]
	// Standard UABE order was: decompressedSize(4) + compressedSize(4) + flags(2)  [10 bytes]
	for (uint32_t i = 0; i < this->blockCount; i++)
	{
		if (!pReader->Read(-1, 2, &blockInf[i].flags))
			goto __goto_readerror;
		SwapEndians_(blockInf[i].flags);
		if (!pReader->Read(-1, 2, &blockInf[i]._aov_unknown))
			goto __goto_readerror;
		// _aov_unknown: do not byte-swap, just consume
		...
		filePos += 12; // AOV: 12 bytes per block (vs 10 in standard)
	}
	...
	if (!dirInf)
		goto __goto_allocerror;
	// UABE_AOV: AOV directory info order = decompressedSize(8) + offset(8) + flags(4) + name
	// Standard UABE order was: offset(8) + decompressedSize(8) + flags(4) + name
...
ASSETSTOOLS_API bool AssetBundleBlockAndDirectoryList06::Write(IAssetsWriter *pWriter, QWORD &curFilePos, AssetsFileVerifyLogger errorLogger)
{
	...
	if (!pWriter->Write(-1, 4, &dwTmp))
		goto __goto_writeerror;
	curFilePos += 4;
	// UABE_AOV: write AOV block info order = flags(2) + unknown(2) + compressedSize(4) + decompressedSize(4)
	for (uint32_t i = 0; i < this->blockCount; i++)
	{
		...
		if (!pWriter->Write(-1, 2, &wTmp))
			goto __goto_writeerror;
		curFilePos += 2;
		uint16_t unkTmp = 0; // _aov_unknown: preserve if set, else 0
		if (!pWriter->Write(-1, 2, &unkTmp))
			goto __goto_writeerror;
		curFilePos += 2;
	...
	for (uint32_t i = 0; i < this->directoryCount; i++)
	{
		// UABE_AOV: write AOV directory order = decompressedSize(8) + offset(8) + flags(4) + name
		QWORD qwTmp = SwapEndians(this->dirInf[i].decompressedSize);
		if (!pWriter->Write(-1, 8, &qwTmp))
			goto __goto_writeerror;
		curFilePos += 8;
		qwTmp = SwapEndians(this->dirInf[i].offset);
		if (!pWriter->Write(-1, 8, &qwTmp))
			goto __goto_writeerror;
		curFilePos += 8;
...
ASSETSTOOLS_API bool AssetBundleHeader06::Read(IAssetsReader *pReader, AssetsFileVerifyLogger errorLogger)
{
	...
	// ----------------------------------------------------------------
	// UABE_AOV: read the 16-byte header block, then read flags.
	// In standard UnityFS: all big-endian.
	// In AOV (flags & 0x200): bytes are reordered then AES-128-CBC
	//   encrypted.  Reorder before decrypt:
	//     in[7..0]  -> reordered[0..7]   (8 bytes reversed)
	//     in[11..8] -> reordered[8..11]  (4 bytes reversed)
	//     in[15..12]-> reordered[12..15] (4 bytes reversed)
	//   After decrypt, all values are little-endian.
	// ----------------------------------------------------------------
	uint8_t hdrBuf[16];
	if (pReader->Read(-1, 16, hdrBuf) != 16)
		goto __goto_readerror;
	...
	if (this->flags & AOV_BUNDLE_ENCRYPT_FLAG)
	{
		// --- Step 1: reorder (matches Python decryptHeader) ---
		uint8_t tmp[16];
		// bytes[0..7] reversed
		for (int i = 0; i < 8; i++) tmp[i] = hdrBuf[7 - i];
		// bytes[8..11] reversed
		for (int i = 0; i < 4; i++) tmp[8  + i] = hdrBuf[11 - i];
		// bytes[12..15] reversed
		for (int i = 0; i < 4; i++) tmp[12 + i] = hdrBuf[15 - i];

		// --- Step 2: AES-128-CBC decrypt ---
#ifdef _WIN32
		if (!AovAES128CBC_Decrypt(kAovHeaderAESKey, kAovHeaderAESIV, tmp, hdrBuf, 16))
		{
			if (errorLogger) errorLogger("AssetBundleHeader06 [AOV]: AES header decryption failed!");
			return false;
		}
#else
		if (errorLogger) errorLogger("AssetBundleHeader06 [AOV]: AES decryption is only supported on Windows.");
		return false;
#endif
		// --- Step 3: parse as little-endian ---
		this->totalFileSize   = (QWORD)hdrBuf[0]        | ((QWORD)hdrBuf[1] << 8)
		                      | ((QWORD)hdrBuf[2] << 16) | ((QWORD)hdrBuf[3] << 24)
		                      | ((QWORD)hdrBuf[4] << 32) | ((QWORD)hdrBuf[5] << 40)
		                      | ((QWORD)hdrBuf[6] << 48) | ((QWORD)hdrBuf[7] << 56);
		this->compressedSize  = (uint32_t)hdrBuf[8]        | ((uint32_t)hdrBuf[9]  << 8)
		                      | ((uint32_t)hdrBuf[10] << 16) | ((uint32_t)hdrBuf[11] << 24);
		this->decompressedSize= (uint32_t)hdrBuf[12]       | ((uint32_t)hdrBuf[13] << 8)
		                      | ((uint32_t)hdrBuf[14] << 16) | ((uint32_t)hdrBuf[15] << 24);
	}
	else
	{
		// Standard big-endian path
		this->totalFileSize = (QWORD)hdrBuf[0] << 56 | (QWORD)hdrBuf[1] << 48
		                    | (QWORD)hdrBuf[2] << 40 | (QWORD)hdrBuf[3] << 32
		                    | (QWORD)hdrBuf[4] << 24 | (QWORD)hdrBuf[5] << 16
		                    | (QWORD)hdrBuf[6] << 8  | (QWORD)hdrBuf[7];
		this->compressedSize  = (uint32_t)hdrBuf[8]  << 24 | (uint32_t)hdrBuf[9]  << 16
		                      | (uint32_t)hdrBuf[10] <<  8 | (uint32_t)hdrBuf[11];
		this->decompressedSize= (uint32_t)hdrBuf[12] << 24 | (uint32_t)hdrBuf[13] << 16
		                      | (uint32_t)hdrBuf[14] <<  8 | (uint32_t)hdrBuf[15];
	}
